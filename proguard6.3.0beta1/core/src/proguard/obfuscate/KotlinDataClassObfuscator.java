...
import proguard.classfile.kotlin.visitors.filter.KotlinFunctionFilter;
 ...
 * name in the visitorInfo field. And that the {@link ClassObfuscator} gives us the
 * new class name (internally this also relies on the new class name being in the
 * visitorInfo field).
 */
class      KotlinDataClassObfuscator
implements KotlinMetadataVisitor
{
    ...
    // Collect the original name/new name map - assumes the old name is in the visitorInfo.
    ...
    implements KotlinPropertyVisitor
    {
        ...
        {
            if (kotlinPropertyMetadata.getVisitorInfo() != null)
            {
                this.nameMap.put(kotlinPropertyMetadata.name + "=", kotlinPropertyMetadata.getVisitorInfo() + "=");
            }
        }
    }
}
